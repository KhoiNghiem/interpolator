output_verilog = 'output_x1.txt'; % Tên file đầu tiên
...
title('Piecewise Parabolic Interpolation Comparison: TF32 vs Verilog vs Ground Truth x1[m] - New architecture ');

figure;
stem(t_interp, abs(x_parabolic_row_tf32_verilog - x_parabolic_row_tf32), 'black', 'LineWidth', 1.5);
xlabel('m');
ylabel('Absolute Error');
title('Absolute Error of Piecewise parabolic Interpolation x1[m] New architecture Bitrue Model Verilog vs Matlab');
grid on;

%% MSE
mse_double = mean((x_parabolic_row_tf32_verilog - x_parabolic_row_tf32).^2);
fprintf('MSE (Verilog vs Matlab new architecture): %.6e\n', mse_double);
